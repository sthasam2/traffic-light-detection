...
# files = get_files("dataset/archive/train_dataset/train_images/*.jpg")
# files = get_files("dataset/S2TLD(720x128）/normal_1/JPEGImages/*.jpg")
files = object_detection.get_files("dataset/empty/*.jpg")
...
for file in files:
    ...
    if not os.path.isdir("traffic_light_cropped/"):
        os.mkdir("traffic_light_cropped/")
    ...
    for idx in range(len(out["boxes"])):
        ...
        if obj_class == object_detection.LABEL_TRAFFIC_LIGHT:
            ...
            cv2.imwrite(
                "traffic_light_cropped/" + str(traffic_light_count) + ".jpg",
                traffic_light,
            )
