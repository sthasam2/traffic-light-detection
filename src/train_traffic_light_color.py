...
from constants import (
    TRAIN_DATASET_DIR_GREEN,
    TRAIN_DATASET_DIR_NOT,
    TRAIN_DATASET_DIR_RED,
    TRAIN_DATASET_DIR_YELLOW,
)  # Handles specialized container datatypes
...
datagen = ImageDataGenerator(
    rotation_range=5,
    width_shift_range=[-10, -5, -2, 0, 2, 5, 10],
    zoom_range=[0.7, 1.5],
...
img_0_green = load_rgb_images(TRAIN_DATASET_DIR_GREEN + "/*", shape)
img_1_yellow = load_rgb_images(TRAIN_DATASET_DIR_YELLOW + "/*", shape)
img_2_red = load_rgb_images(TRAIN_DATASET_DIR_RED + "/*", shape)
img_3_not_traffic_light = load_rgb_images(TRAIN_DATASET_DIR_NOT + "/*", shape)
...
checkpoint = ModelCheckpoint(
    "traffic.h5", monitor="val_loss", mode="min", verbose=1, save_best_only=True
)
...
model.compile(
    ...
    optimizer=Adadelta(learning_rate=1.0, rho=0.95, epsilon=1e-08, weight_decay=0.0),
    metrics=["accuracy"],
)
...
for idx in range(len(x_valid)):
    ...
    file_name = (
        str(idx) + "_" + str(label) + "_" + str(np.argmax(str(y_valid[idx]))) + ".jpg"
    )
    img = img_as_ar[0]
